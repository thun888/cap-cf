...
const countryFlags = (code) => {
  ...
  return `<img src="${url(`assets/flags/${code.toLowerCase()}.svg`)}" alt="${code}" class="country-flag" onerror="this.style.display='none'">`;
};
...
async function loadWorldMap() {
  ...
  if (!window.topojson) {
    await new Promise((resolve, reject) => {
      ...
      s.src = url("assets/topojson-client.min.js");
      s.onload = resolve;
      s.onerror = reject;
      document.head.appendChild(s);
    });
  }
  worldTopoData = await (await fetch(url("assets/countries-110m.json"))).json();
...
async function openSettings() {
  ...
  const modal = createModal(
    ...
    <div class="settings-content">
      ...
      <div class="settings-section" id="aboutSection">
        ...
        <a href="https://github.com/thun888/cap-cf" target="_blank" class="github-link">
  ...
  if (about.version) {
    document.getElementById("aboutInfo").innerHTML =
      `Cap <b>v${escapeHtml(about.version)}</b><br>${escapeHtml(about.runtime || '')}${about.demo ? '<br><span style="color:var(--blue);font-weight:500">Demo mode</span>' : ''}<br><br><span style="color:var(--text-muted);font-size:13px">移植自 <a href="https://github.com/tiagozip/cap/" target="_blank" style="color:var(--blue)">Cap</a></span>`;
  }
}
